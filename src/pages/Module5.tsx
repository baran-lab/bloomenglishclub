...
import { Module5Complete } from '@/components/module5/Module5Complete';
...
const Module5Content: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!lessonId) return;
    const target = module5Lessons.find(l => l.id === lessonId);
    if (target) {
  ...
  const handleLessonClick = (lesson: Module5Lesson) => {
  ...
  const renderLessonContent = (lesson: Module5Lesson) => {
    switch (lesson.type) {
      case 'vocabulary':
      ...
      case 'listening-fill-blank':
        return <ListeningFillInBlankM5 items={lesson.fillItems || []} onComplete={handleLessonComplete} title={lesson.title} />;
      case 'video-series':
      ...
      case 'module-complete':
        return <Module5Complete onComplete={handleLessonComplete} />;
      default:
        return null;
    }
  };
      ...
      <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="container max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-fredoka text-lg font-bold text-primary">Module 5: Food & Cooking</h1>
          ...
          {viewState === 'language-select' && (
            <motion.div key="language-select" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
              <div className="text-center space-y-2">
                <h2 className="font-fredoka text-3xl font-bold text-foreground">Module 5: Food & Cooking 🍳</h2>
                <p className="text-muted-foreground max-w-md mx-auto">Learn food vocabulary, grocery items, and kitchen words.</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {lessons.map((lesson, index) => (
                    <LessonCard key={lesson.id} lesson={lesson as any} index={index} onClick={() => handleLessonClick(lesson)} />
                  ))}
                </div>
              </div>
            </motion.div>
