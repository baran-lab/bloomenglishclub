...
interface OrdinalAudioMatchingPracticeProps {
...
export const OrdinalAudioMatchingPractice: React.FC<OrdinalAudioMatchingPracticeProps> = ({ 
  content, 
  onComplete,
  ...
  const progress = (matchedPairs.size / content.length) * 100;
  const allCompleted = matchedPairs.size === content.length;

  const shuffleArrays = useCallback(() => {
    setShuffledAudio([...content].sort(() => Math.random() - 0.5));
    setShuffledNumbers([...content].sort(() => Math.random() - 0.5));
  ...
  const speakOrdinal = (item: VocabularyItem) => {
    if ('speechSynthesis' in window) {
      speechSynthesis.cancel();
      // Extract the word part (e.g., "First" from "1st - First")
      const word = item.english.split(' - ')[1] || item.english;
  ...
  // Extract number part (e.g., "1st" from "1st - First")
  const getNumberPart = (english: string) => english.split(' - ')[0];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">Listen to the audio and match with the correct number</p>
      </div>
        ...
        <div className="space-y-2">
          ...
          {shuffledAudio.map((item) => (
            ...
            >
              ...
              {matchedPairs.has(item.id) ? getNumberPart(item.english) : 'Play'}
