...
const ahmedVideos = [
  { url: '/videos/module2/ahmed-1.mp4', question: 'What is his first name?', options: ['Ahmed', 'Ali'], correctAnswer: 0 },
  { url: '/videos/module2/ahmed-2.mp4', question: 'Where is he from?', options: ['Turkey', 'Egypt'], correctAnswer: 1 },
  { url: '/videos/module2/ahmed-3.mp4', question: 'How old is he?', options: ['35', '40'], correctAnswer: 0 },
  { url: '/videos/module2/ahmed-4.mp4', question: 'Is he married or single?', options: ['Married', 'Single'], correctAnswer: 0 },
  { url: '/videos/module2/ahmed-5.mp4', question: 'What does he do?', options: ['Uber Driver', 'Electrician'], correctAnswer: 0 },
  { url: '/videos/module2/ahmed-6.mp4', question: 'Where does he live?', options: ['Apartment 3A', 'Apartment 4B'], correctAnswer: 0 },
];
...
export const AhmedNeighborQuiz: React.FC<AhmedNeighborQuizProps> = ({ onComplete }) => {
  ...
  const currentVideo = ahmedVideos[currentIndex];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h3 className="font-fredoka text-xl font-bold text-foreground">Meet Ahmed El-Masri 👨‍🦱</h3>
        <p className="text-sm text-muted-foreground mt-1">
          Watch and answer - Video {currentIndex + 1} of {ahmedVideos.length}
        </p>
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        {showResult && (
          <Button onClick={handleNext} className="gap-2">
            {currentIndex < ahmedVideos.length - 1 ? 'Next Video' : 'Complete'}
            <ChevronRight className="w-4 h-4" />
          </Button>
        )}
      </div>

      {currentIndex === ahmedVideos.length - 1 && showResult && (
        <motion.div
        ...
        >
          ...
          <p className="text-lg font-bold text-green-600">
            You scored {score} out of {ahmedVideos.length}!
          </p>
        </motion.div>
      )}
    </div>
