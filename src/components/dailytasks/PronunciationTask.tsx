...
import type { PronunciationContent } from "@/data/dailyTaskContent";

interface Props {
  task: PronunciationContent;
  onComplete: () => void;
}

export function PronunciationTask({ task, onComplete }: Props) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [completed, setCompleted] = useState<Set<number>>(new Set());
  const word = task.words[currentIndex];

  const handleListen = () => speakText(word, 0.6);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-muted rounded-xl p-6 text-center space-y-4">
        ...
        {completed.has(currentIndex) && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-success font-semibold text-sm">👏 Well done!</motion.p>
        )}
      </div>
